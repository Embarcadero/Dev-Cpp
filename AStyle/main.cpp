...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam,LPARAM lParam) {
	switch(Message) {
		...
		case WM_CREATE: {
			// Create fancy font
			HDC hdc = GetDC(hwnd);
			HFONT font = CreateFont(-MulDiv(10,GetDeviceCaps(hdc, LOGPIXELSY),72),0,0,0,0,0,
			...
			SendMessage(EnglishFileText,WM_SETFONT,(WPARAM)font,0);
			...
			LogBoxHandle = CreateWindowEx(WS_EX_CLIENTEDGE,"EDIT","",
			                              ...
			                              (HMENU)ID_LOGBOX,
			                              GetModuleHandle(NULL),NULL);
			SendMessage(LogBoxHandle,WM_SETFONT,(WPARAM)font,0);
			break;
		}
